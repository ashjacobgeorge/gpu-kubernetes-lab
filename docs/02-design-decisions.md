...
You can have a cluster running in under 5 minutes.
...
- How etcd is bootstrapped
- How nodes join securely using tokens
...
- Quick proof of concept that doesn't need production fidelity
...
- Lets us focus on learning Kubernetes, not debugging networking
...
SR-IOV + RDMA - for GPU-to-GPU communication on real hardware
  - Bypasses the kernel entirely
  - Near wire-speed between GPUs on different nodes
  - Used by CoreWeave, Lambda Labs for distributed training

---

## Why Ubuntu 22.04 over other Linux distributions

| Distribution    | Rejected reason                                    |
|-----------------|----------------------------------------------------|
| Ubuntu 22.04    | CHOSEN                                             |
...
updates until 2027. Most Kubernetes documentation assumes Ubuntu.
...
join the cluster:
    dial tcp 192.168.64.2:6443: connect: no route to host

### Why shared networking works
Shared networking puts all VMs on the same bridge network
(192.168.105.x). Every VM gets a different IP in that range
and can reach every other VM directly. This is required for:
